...
test("coach can log in and reach dashboard", async ({ page }) => {
  ...
  // Stats card with client count visible
...
test("trainee can log in and reach today page", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/trainee\/today/, { timeout: 10000 });
});
...
test("trainee today page renders workout or empty state", async ({ page }) => {
  ...
  await page.waitForLoadState("networkidle");
  // Either a workout card with Log Set button or the no-plan empty state
  const hasWorkout = await page.getByRole("button", { name: /log set/i }).count();
  const emptyState = await page.getByText(/ask your coach/i).count();
  expect(hasWorkout + emptyState).toBeGreaterThan(0);
});
...
test("trainee generator page loads", async ({ page }) => {
  await loginAs(page, TRAINEE_EMAIL, TRAINEE_PASSWORD);
  await page.goto("/trainee/generator");
...
test("trainee is redirected away from /coach/dashboard", async ({ page }) => {
  await loginAs(page, TRAINEE_EMAIL, TRAINEE_PASSWORD);
  await page.goto("/coach/dashboard");
  await expect(page).not.toHaveURL(/\/coach\/dashboard/, { timeout: 8000 });
});
...
test("coach plan builder page loads for a client", async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/coach\/clients\/.+\/plan/, { timeout: 8000 });
