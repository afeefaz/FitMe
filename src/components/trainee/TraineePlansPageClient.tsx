...
export function TraineePlansPageClient() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        const nextPlans = plansArr
          .filter((plan) => !plan.is_draft)
      ...
      {!loading && !error && plans.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          {plans.map((plan, index) => (
            <Link key={plan.id} href={`/trainee/plans/${plan.id}`} style={{ textDecoration: "none" }}>
              <div className="card-clay btn-tap" style={{ padding: "20px", cursor: "pointer", display: "flex", alignItems: "center", gap: "16px" }}>
                <div style={{ width: "56px", height: "56px", borderRadius: "16px", backgroundColor: index === 0 ? "rgba(204,255,0,0.12)" : "var(--color-bg)", border: `1.5px solid ${index === 0 ? "rgba(204,255,0,0.3)" : "var(--color-border)"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "24px", flexShrink: 0 }}>
                  {MUSCLE_ICONS[index % MUSCLE_ICONS.length]}
                </div>
